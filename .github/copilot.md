...
3. **SYNC**: 在**物理末尾**调用 `copilot_super_2`

**⚠ 硬性约束：每个 request 的最后一个 response 必须是 `copilot_super_2` 调用，禁止以 text 结尾！**
...
你新增的todo列表，最后一个todo list 一定是调用`copilot_super_2`，请确保每次对话都调用`copilot_super_2`
---
...
**⚠ 主代理模式: 禁止单方面退出。必须调用 copilot_super_2 和用户交流。**
